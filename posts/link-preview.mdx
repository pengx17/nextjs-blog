...
[link-preview-js](link-preview-js) 这个 Node.js 库可以方便的实现我们的需求，把目标 URL 的相关特征，以 JSON 格式输出。因为跨域问题，我们需要把它部署在服务端，并且提供一个简单的接口，让客户端获取相关信息。
...
测试中我们发现，`link-preview-js` 对客户端渲染的页面提取效果不好。对于 twitter.com/pengx17，我们在页面渲染完成后，可以通过 JS 获取相关 OGP 信息：
...
但实际上 `link-preview-js` 只会返回 `og:site_name` 一个属性，这是因为页面在服务端渲染的时候并没有把所有的 og 标签都渲染出来。

是否有比较优雅的解决方案呢？这是一个值得思考的问题。
