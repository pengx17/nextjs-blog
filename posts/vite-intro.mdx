...
Vite 的配置规<Note label="借鉴">有兴趣的朋友可以看一下这个[推特](https://twitter.com/youyuxi/status/1460406787757072386?s=21)上的八卦。</Note>了 Parcel 的 Zero configuration (即开箱即用) 的概念，降低了配置前端项目的复杂度。
...
不过上述写法会导致 sourcemap 失效。这个问题在 https://github.com/aleclarson/vite/pull/3 进入 vite 主干分支后解决 <Note>已经进入主干分支，我还贡献了一点代码 😏</Note>。
