...
Qiankun 在其主页上声称，它是“简单、完备、生产可用”的。
理论上讲，Qiankun 可以让一个前端应用在不做专门适配的情况下，就可以注册为子应用，嵌入到一个微前端应用的基座容器中正常使用。

将任意的 Web 应用直接作为微前端子应用有一些技术难点需要攻克，比如：
...
框架的抽象爆露出来的外部 API 与实现之间的权衡中，Qiankun 试图把沙箱的实现复杂度封装在框架内部，以便于开发者可以把任务中心放在业务实现上，尽量减轻沙箱机制对于开发者造成的心智负担。

一个老的页面本身没有考虑到会被以微前端子应用使用，自然地它也不会对全局变量的使用有任何顾虑。以 Qiankun 的愿景来看，它不希望子应用那边需要做过多的改动，那么就需要在运行时多做一点文章。

## 初探运行时沙箱实现原理

iframe 是一种特性完备的浏览器沙箱。但 Qiankun 设计者认为，iframe 本身有一些体验上的问题，因此从最开始就排除了将 iframe 作为核心的微前端方案。

根据隔离场景，我们需要有 JS 沙箱与 CSS 沙箱两种机制。

### JS 沙箱

JS 沙箱目前还没有主流浏览器支持的成熟方案。TC39 标准的 [ShadowRealm API](https://tc39.es/proposal-shadowrealm/) 提案未来可期，社区中有一些 Polyfill 实现，不过目前还处于很早期的草案阶段，很难作为生产环境方案使用。
...
Qiankun 使用了一种巧妙（Hack）的方式在运行时对子应用的 JS 模块中全局变量访问进行重定向。
...
另外，CSS-in-JS 框架，如 Emotion/styled-components、或是 Webpack 的 style-loader 会把样式在 JS 中动态的向根部页面的 HEAD 标签插入 STYLE。这里有两个问题：

- 子应用卸载时，也需要清理这些动态样式标签
