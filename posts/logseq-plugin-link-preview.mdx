...
[在前面的博客](/posts/link-preview)里，我介绍了将任意外链渲染为卡片模板的方法。为了在 Logseq 里实现两种渲染模式，我们还需要解决其他问题。
...
<video
  src="/videos/link-preview-plugin-hover-demo.mp4"
  width="100%"
  loop
  autoPlay
></video>
...
> 这里请注意，发布在市场中的 Logseq 插件默认情况是运行在独立域名中的 `iframe` 沙盒里，访问主视图 `top` 引用（指向到主视图的 `window` 对象）的能力被[跨域安全策略](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy#cross-origin_script_api_access)禁止，我们目前只有在本地通过 `Load unpacked plugin` 的方式加载插件，才能访问到 `top` 引用。
...
Logseq 中的宏与[编程语言的宏](<https://en.wikipedia.org/wiki/Macro_(computer_science)>)概念类似，其作用就是将输入文本替换为其他的文本。我们的接下来的目标就是将 `{{renderer :linkpreview,http://www.baidu.com}}` 文本渲染成相应 HTML 模板：
